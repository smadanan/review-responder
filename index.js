...
function httpsGet(hostname, apiPath) {
  return new Promise((resolve, reject) => {
    const req = https.request(
      { hostname, path: apiPath, method: "GET", headers: { "Accept": "application/json" } },
      (res) => {
        let data = "";
...
async function getPasswords() {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      method: "GET",
      headers: { "Authorization": `Bearer ${GITHUB_TOKEN}`, "User-Agent": "review-responder", "Accept": "application/vnd.github.v3+json" }
    };
...
async function savePasswords(clients, sha) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      method: "PUT",
      headers: {
        "Authorization": `Bearer ${GITHUB_TOKEN}`, "User-Agent": "review-responder",
...
function randomString() { return Math.random().toString(36).substring(2, 6); }
...
app.get("/app", isAuthenticated, (req, res) => res.sendFile(path.join(__dirname, "public", "index.html")));
app.get("/admin", (req, res) => res.sendFile(path.join(__dirname, "public", "admin.html")));

// ─── Auth Routes ──────────────────────────────────────────────────────────────
app.get("/auth/google", passport.authenticate("google", { scope: ["profile", "email"], accessType: "online" }));
...
app.get("/places/search", isAuthenticated, async (req, res) => {
  ...
  try {
    const data = await httpsGet(
      "maps.googleapis.com",
      `/maps/api/place/textsearch/json?query=${encodeURIComponent(query)}&key=${GOOGLE_PLACES_API_KEY}`
    );
    ...
    const places = (data.results || []).slice(0, 6).map(p => ({
      ...
      address: p.formatted_address, rating: p.rating,
...
app.post("/admin/clients", async (req, res) => {
  ...
  try { const { clients } = await getPasswords(); res.json({ clients }); }
...
app.post("/admin/reset", async (req, res) => {
  ...
  if (adminPassword !== ADMIN_PASSWORD) return res.status(401).json({ error: "Unauthorized" });
  if (!newPassword || newPassword.trim().length < 4) return res.status(400).json({ error: "Min 4 characters." });
  try {
    const { clients, sha } = await getPasswords();
    const client = clients.find(c => c.id === id);
    if (!client) return res.status(404).json({ error: "Client not found." });
    client.password = client.currentPassword = newPassword.trim();
    await savePasswords(clients, sha);
    res.json({ success: true });
...
app.post("/admin/toggle", async (req, res) => {
  ...
  try {
    ...
    client.currentPassword = client.active ? client.password : randomString();
    await savePasswords(clients, sha);
...
app.post("/generate", isAuthenticated, async (req, res) => {
  const {
    businessName, businessType, review, tone, stars,
    language = "English",
...
- Keep it between ${minWords} to ${maxWords} words
...
- End with a warm, forward-looking closing${customInstructions ? `\n- ${customInstructions}` : ""}
